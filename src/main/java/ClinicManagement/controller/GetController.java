...
public class GetController implements IGetRequests {
    ...
    @Override
    @GetMapping("/medicineTimes/patient/{id}")
    public ResponseEntity<JSONArray> getMedTimesForPatient(@PathVariable int id) {
        logger.info("getting MedicineTimes ..");
        return new ResponseEntity<>(JsonFactory.toGetMedTimes(medicineTimeService.getMedTimesForPatient(id, true)), HttpStatus.OK);
    }

   /* @Override
    @GetMapping("/medicineTimes")
    public ResponseEntity<JSONArray> getMedTimes() {
        logger.info("getting Appointments ..");
        return new ResponseEntity<>(JsonFactory.toGetMedTimes(medicineTimeService.getMedTimes(true)), HttpStatus.OK);
    }*/
    ...
    public ResponseEntity<JSONArray> getAppointmentByDate(
       ...
       // logger.info("getting Appointment By Date AND Time : { " + date + " , " + time + " }");

        Date sqlDate = Date.valueOf(date.toLocalDate());
        Time sqlTime = Time.valueOf(date.toLocalTime());
        System.out.println("Time : "+sqlTime+"   "+"Date : "+sqlDate);
    ...
    public ResponseEntity<JSONArray> getAppointmentsByPatientIdAndPendingState(
        ...
        /*http://HOST_NAME:PORT/api/get/appointments/status/pending/patient/date/?date=yyyy-MM-dd&id=0*/
    ...
    public ResponseEntity<JSONArray> getAppointmentsByPatientIdAndAcceptState(
            @RequestParam("id") int patientId,
            @RequestParam(value = "date") @DateTimeFormat(pattern = "yyyy-MM-dd") java.util.Date date)  {
        /*http://HOST_NAME:PORT/api/get/appointments/status/accept/patient/date/?date=yyyy-MM-dd&id=0*/
        Date sqlDate = new Date(date.getTime());
    ...
    public ResponseEntity<JSONArray> getAppointmentsByPatientIdAndRefuseState(
            @RequestParam("id") int patientId,
            @RequestParam(value = "date") @DateTimeFormat(pattern = "yyyy-MM-dd") java.util.Date date)  {
        /*http://HOST_NAME:PORT/api/get/appointments/status/refuse/patient/date/?date=yyyy-MM-dd&id=0*/
        Date sqlDate = new Date(date.getTime());
    ...
    public ResponseEntity<JSONArray> getAppointmentsForDoctorAndByPendingStatusAndDate(
            ...
            @RequestParam(value = "date") @DateTimeFormat(pattern = "yyyy-MM-dd") java.util.Date date) {
        /*http://HOST_NAME:PORT/api/get/appointments/status/pending/doctor/date/?date=yyyy-MM-dd&id=0*/
    ...
    /**
     * the link should be modfiy to the same as it siblings
     */
...
//        ByteArrayResource resource = new ByteArrayResource(Files.readAllBytes(p));
    ...
    @GetMapping("/patient/medicine/patient")
    public ResponseEntity<JSONObject> getPatientMedicine(@RequestParam("medicineID") int medicineID,@RequestParam("patientID") int patientID)
    {
