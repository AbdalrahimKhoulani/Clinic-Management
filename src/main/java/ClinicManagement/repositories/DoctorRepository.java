...
public interface DoctorRepository extends JpaRepository<Doctor, Integer> {
    ...
    // @Query("SELECT new ClinicManagement.entity.Patient(p.patient_id,p.name) FROM Patient p JOIN Appointment a ON a.patient.patient_id =p.patient_id WHERE a.doctor.doctor_id = :id")
   ...
   /* @Query("SELECT d FROM Doctor d JOIN d.workTimeDoctors wt" +
            " WHERE wt.workDays.name =:dayName" +
            " AND d.isActive =:isDoctorActive")
    List<Doctor> findByTime(@Param("dayName") String workDay_name,@Param("isDoctorActive") boolean isDoctorActive);*/
